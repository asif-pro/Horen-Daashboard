export function getWeekBracket(){
    ...
    if(currentDay > 0 && currentDay<=7) return { startDate : `${currentYear}-${currentMonth}-01`, endDate : `${currentYear}-${currentMonth}-07`}
    if(currentDay > 7 && currentDay<=15) return { startDate : `${currentYear}-${currentMonth}-08`, endDate : `${currentYear}-${currentMonth}-15`}
...
export function cleanDataByTime(timeFrame, data){
    ...
    const datesWithDataPoints = data.filter((item)=>{
      ...
      // console.log(datesWithDataPoints)
      timeSeriesData.forEach((day, timeSeriesDataKey)=>{
        datesWithDataPoints.forEach((date, datesWithDataPointsKey)=>{
          if (day.date==date._id.split("-")[2]) timeSeriesData[timeSeriesDataKey].horns = Number(datesWithDataPoints[datesWithDataPointsKey].horn_count)
        })
      });
